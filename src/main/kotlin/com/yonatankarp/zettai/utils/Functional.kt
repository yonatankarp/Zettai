 ...
 */
infix fun <A : Any, B : Any, C : Any> FUN<A, B?>.andUnlessNull(other: FUN<B, C?>): FUN<A, C?> = { a: A -> this(a)?.let { other(it) } }

/**
 * Applies the given function to the sequence if it is not null or return null if it does.
 */
fun <U : Any> CharSequence?.unlessNullOrEmpty(f: (CharSequence) -> U): U? = if (isNullOrEmpty()) null else f(this)
